...
def initializeSlaveClient(port=8080):

    slave_client=socket.socket()
    slave_client.connect('127.0.0.1',port)

    print("start to sending time to server")
    send_time_thread=threading.Thread(target=startSendingTime,args=(slave_client,))
    send_time_thread.start()

    print("start receiving synchronized time from server ")
    recieve_time_thread=threading.Thread(target=startReceivingTime,args=(slave_client,))
    recieve_time_thread.start()


if __name__=="__main__()":
    initializeSlaveClient(port=8080)
